    ...
    public boolean check(SymbolNode pTable) {
        ...
        if (((Expr)(operandsList.get(1))).expType.equals(((Expr)(operandsList.get(0))).expType)){
            if (!((Expr)(operandsList.get(1))).expType.equals(INTEGER_TYPE)){
                Program.addError(new MyException("one or two sides of the equal operation is not integer",this));
                result = false;
            }
        }
        else{
            Program.addError(new MyException("two sides of the equal operation do not have the same type",this));
            result = false;
        }
